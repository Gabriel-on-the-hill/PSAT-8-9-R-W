...
// The order the four domains appear in a real digital PSAT 8/9 R&W module. R&W
// is NOT shuffled the way the Math module is: the section runs domain by
// domain, and within a domain the questions climb from easy to hard. Anything
// we build that claims to be "test-style" is ordered by this, so there is one
...
// ══════════════════════════════════════════════════════════════════
// RATIO MIX  (custom practice in a proportion)
//
...
//
// Do not replace the switch with a heuristic on whether the numbers differ.
// That costs you the even split: 1-and-1 is how you would ask for five and
// five, and it is also exactly what an untouched screen looks like, so the
// request becomes unaskable and fails silently as "whatever the queue had" —
// which for two skills is frequently ten of one and none of the other.
// ══════════════════════════════════════════════════════════════════
...
//
// `pool` arrives in the order prioritizePool built — misses first, then unseen,
// then resting by how overdue they are. Every pass below walks it in that order
// and only ever filters, so within a quota the student still gets the questions
// he most needs. The ratio decides how MANY of each; the queue decides WHICH.
//
...
// in a way that only shows up in the interior: five of each skill and five of
// each difficulty — both marginals exactly right — and yet almost all the Hard
// landed on one skill. Nothing is wrong with the arithmetic; the first pass
// simply walks the queue and whichever cell it meets first eats the allowance.
// A tutor who sets both dimensions means "half of EACH skill hard", and that
// is a claim about the cells.
//
// So: apportion by skill first, then apportion each skill's quota by
// difficulty. The grid is the target; the marginals fall out of it. The cost is
// that per-skill rounding lets the overall difficulty totals drift off the
// stated ratio (1:3 over 12 lands 4/8, not 3/9). That is the right trade.
//
...
//
// This is where R&W deliberately parts company with a Math app, which shuffles
// its custom set — Math genuinely is presented mixed. Shuffling here would
// teach a question order the student will never meet.
//
...
function buildActiveQuestions() {
    ...
    if (limit > 0) {
        ...
        // answered aren't the ones pulled back into this set. With a ratio on
        // this is also the candidate queue it draws from: a quota must not be
        // denied just because that skill's due questions ran out this week.
        ...
        if (!ratioOn) {
            // Unchanged: due questions first, top up only if they run short.
...
function finalizeSession() {
    ...
    // answered" label at the missed-list was written for a case that could not
    ...
    if (mockExam) {
        // Between modules: stash module 1, launch module 2 (no score shown).
        if (mockModuleIdx === 0 && mockModules[1] && mockModules[1].length) {
            mockResults = sessionResults.slice();
            mockMissed  = missedQuestions.slice();
            startMockModule(1);
            return;
        }
        // Final module done — combine both modules for the result.
        sessionResults  = mockResults.concat(sessionResults);
        missedQuestions = mockMissed.concat(missedQuestions);
        const correct   = sessionResults.filter(r => r.isCorrect).length;
        logSession(['Mock PSAT R&W'], ['Easy', 'Medium', 'Hard'], correct, sessionResults.length);
        mockScoreEstimate = estimateRWScore(correct, sessionResults.length);
        mockExam = false;
        showCompletion();
...
// ══════════════════════════════════════════════════════════════════
// MOCK PSAT 8/9 — READING & WRITING  (full section: two modules)
// Mirrors the real digital R&W section: 2 modules × 27 questions, 32 minutes
// each (64 total), domain order following the College Board blueprint, a
// realistic easy/medium/hard spread, exam conditions (no feedback, blind
// score), and an estimated 120–720 R&W section score at the end.
// Note: the real test routes module 2 by module-1 performance (adaptive);
// this practice mock uses two fixed modules + a combined estimate instead.
// ══════════════════════════════════════════════════════════════════

const MOCK_MODULE_SIZE    = 27;
const MOCK_MODULE_SECONDS = 32 * 60;            // 32 minutes per module
// Per module → ≈ blueprint weights. The ORDER here is not the presentation
// order — orderSATStyle owns that, via RW_DOMAIN_ORDER. Keeping the two in step
// by eye is how they drift, so this only says how many.
...
let mockModules       = [];   // [module1[], module2[]]
let mockModuleIdx     = 0;
let mockResults       = [];   // accumulated sessionResults across modules
let mockMissed        = [];   // accumulated misses across modules

// Build one 27-question module to the blueprint mix, with a given easy/med/hard
// spread, excluding ids already used (so module 2 never repeats module 1).
function buildMockModule(spread, used) {
    const out  = [];
...
function buildMockExam() {
    const used = new Set();
    const m1 = buildMockModule({ E: 0.40, M: 0.45, H: 0.15 }, used); // gauge module
    const m2 = buildMockModule({ E: 0.25, M: 0.45, H: 0.30 }, used); // slightly harder
    mockModules   = [m1, m2];
    mockModuleIdx = 0;
    mockResults   = [];
    mockMissed    = [];
...
function startMockModule(idx) {
    if (idx === 1) {
        alert('Module 1 complete.\n\n' +
              'Take a short break if you need one.\n\n' +
              'Module 2 has 27 questions and 32 minutes. It starts when you click OK.');
    }
    launchSession(mockModules[idx], 'exam', { mode: 'countdown', total: MOCK_MODULE_SECONDS });
    mockExam      = true;   // launchSession clears this; set it after.
    mockModuleIdx = idx;
}

function startMockExam() {
    if (!confirm(
        'Mock PSAT 8/9 — Reading & Writing\n\n' +
        '54 questions in two modules · 32 minutes each (64 total) · timed, ' +
        'with no feedback until the end — just like the real test.\n\n' +
        "You'll get an estimated R&W score (120–720) at the finish.\n\n" +
        'Start now?')) return;
    buildMockExam();
    if (!mockModules[0] || mockModules[0].length === 0) {
        alert('Not enough questions available for a mock right now.'); return;
    }
    startMockModule(0);
}

// Approximate PSAT 8/9 R&W section score (120–720) from raw correct out of 54.
// A linear estimate anchored to the score range — approximate, not official.
function estimateRWScore(correct, total) {
    const pct = total ? correct / total : 0;
    return Math.round((120 + pct * 600) / 10) * 10;
}
...
function updateSetupUI() {
    ...
    if (filtered.length === 0) {
        ...
        if (allMastered) {
            ...
            const body = ratioOn ? describeMix(built) : labels.join(' + ');
...
function showCompletion() {
    const isMock  = mockScoreEstimate != null;
    const total   = isMock ? sessionResults.length : activeQuestions.length;
    const correct = isMock ? sessionResults.filter(r => r.isCorrect).length : score;
    ...
    document.getElementById('completionBigScore').textContent = `${correct} / ${total}`;
    ...
    if (mockEl) {
        if (mockScoreEstimate != null) {
            mockEl.innerHTML = `Estimated PSAT R&amp;W score &nbsp;<b>~${mockScoreEstimate}</b>` +
                ` <span class="mock-score-note">/ 720 · approximate</span>`;
...
function loadQuestion(index) {
    ...
    document.getElementById('passageContent').innerHTML =
        q.image ? renderQuestionImage(q.image, q.alt) : formatPassage(q.passage, q.skill);
...
function renderQuestionImage(src, alt) {
    return `<img class="full-q-image" src="${escapeHtml(src)}" alt="${escapeHtml(alt || 'Question figure')}" loading="lazy">`;
}
...
//
// This used to be the whole answer pipeline: first click wrote the mastery
