...
const now = () => Date.now();
function user(u) { ctx.sessionStorage.setItem('psat89_user', u); }
function setLedger(led, u) {
    ctx.localStorage.setItem('psat89_progress_' + (u || 'guest'), JSON.stringify(led));
}
function getLedger(u) {
    return JSON.parse(ctx.localStorage.getItem('psat89_progress_' + (u || 'guest')) || '{}');
}
...
section('13 · Multi-student: one frozen set, independent progress, zero new storage');
{
    ...
    ok('no challenge-specific storage key exists',
        ctx.localStorage.getItem('psat89_challenge_p8') === null);
}
