...
const SCRIPTS = [
    ...
    null,
    // No data-challenge-*.js in this app. Those carry the verbatim questions a
    // student missed on a marked practice test, for the unscored debrief layer.
    // This app's sets are built from the bank by concept coverage, so they have
    // ids and no `review` block, and there is nothing to declare.
...
// debrief assertions below are skipped rather than asserted against nothing.
...
const ledger = w => JSON.parse(w.localStorage.getItem('psat89_progress_' + STUDENT) || 'null');

async function main() {
// ═════════════════════════════════════════════════════════════════
section('1 · The tile appears only for a student who has a set');
{
    const w = await build(STUDENT);
    const tile = $(w, 'challengeTile');
    ok(STUDENT + ' gets a tile', !!tile);
...
section('2 · The start screen and its gates');
{
    ...
    const ids = w.CHALLENGE_SETS[STUDENT][w.CHALLENGE_SETS[STUDENT].length-1].ids;
    const led = {};
    ids.forEach(id => { led[id] = { correct: 1, wrong: 0, lastSeen: Date.now() }; });
    w.localStorage.setItem('psat89_progress_' + STUDENT, JSON.stringify(led));
    w.openChallenge();
...
// ═════════════════════════════════════════════════════════════════
// Layer 1 only exists for a set built from a marked practice test. Every set in
// this app so far is built from the bank by concept coverage, so there is nothing
// to debrief and this section has no subject. It is kept, not deleted: the day a
// set here does carry a `review` block, these are the assertions that must hold.
section('3 · Layer 1 (debrief) never touches the mastery ledger');
if (REVIEW_N === 0) {
    console.log('  – no set in the roster carries a review block; debrief layer not exercised');
} else {
    const w = await build(STUDENT);
    w.openChallenge();
    $(w, 'cDebriefBtn').click();
    ...
    const before = w.localStorage.getItem('psat89_progress_' + STUDENT);
    ...
    eq('ledger untouched', w.localStorage.getItem('psat89_progress_' + STUDENT), before);
...
section('4 · The exam-mode guard  (the reason this test exists)');
{
    ...
    w.localStorage.removeItem('psat89_progress_' + STUDENT + '');
...
{
    const plain = await build(STUDENT);
    eq('no deep link → hub stays up', $(plain, 'hubScreen').style.display, 'flex');

    const deep = await build(STUDENT, '?challenge=1');
