...
t('marks itself INCOMPLETE in focus, which is the only cell that can carry it', () => {
    const ctx = makeCtx();
    ctx.setSessionId('sid-3');
    seed(ctx, 2, [0, 1]);
    ctx.logPartialSession();
    const f = ctx.posted[0].focus;
    ok(/INCOMPLETE/.test(f), 'focus must say INCOMPLETE, got: ' + f);
    ok(/2 of 6/.test(f), 'focus must show the shortfall, got: ' + f);
...
t('a resumed session keeps its id, so a partial and its finish are joinable', () => {
    ...
    eq(ctx.posted[0].sessionId, ctx.posted[1].sessionId, 'ids must match so the pair resolves:');
});
...
t('sheet-sync.js names focus, partial and sessionId', () => {
    ...
    ['focus', 'partial', 'sessionId'].forEach(k => {
...
t('homework-run.html flushes a part-answered set, and COMMITS a fully answered one', () => {
    ...
    // 6 Sep 2026. The partial flush was written for a set abandoned halfway and it
    ...
    // answered)" — not a description of anything — and her completion flag was still
    // never written, which under sequential unlock shut every later set behind it.
    // That is the seventeen-day outage, and it was invisible: the score screen had
    // already shown her a finished-looking result and the tutor read it with her.
    //
    // Reaching the score screen is how the STUDENT sees her result. It is not what
    // makes the work exist.
    ok(/answered\s*>=\s*daySet\.length/.test(handler),
       'pagehide must recognise a fully answered set');
    ok(/localStorage\.setItem\('psat89_hw_'/.test(handler),
