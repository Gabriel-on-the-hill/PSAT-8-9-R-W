...
// setup screen), and the module-style ordering of the set it produces.
//
...
// <script> element rather than eval()ing it — lexical globals like
// `const questionBank` are scoped away by eval and would never appear.
//
...
//      Five-and-five is the most ordinary thing a tutor asks for. Reading
//      intent off whether the numbers differ makes that request
//      indistinguishable from an untouched screen; this one does not.
//
...
//      interior wrong: five of each skill and five of each difficulty, with
//      four of the five Hard sitting on ONE skill. The marginals prove nothing
//      about the cells, and "half of each skill hard" is a claim about cells.
//      §3b is what stops that regressing.
//
//   4. The set comes out in R&W module order: domain blocks in
//      RW_DOMAIN_ORDER, easy → hard inside each. A Math app shuffles its custom
//      set because SAT/PSAT Math is presented mixed; R&W is not, and copying
...
// `mockModules` is `let`, so it is lexical too — and it is what buildMockExam
// writes its result into. Read through the probe, not off window.
const PROBE = 'window.__peek = function(){ return { SKILL_DOMAIN, RW_DOMAIN_ORDER, questionBank }; };'
            + 'window.__mockModules = function(){ return mockModules; };';
...
function build() {
    return new Promise(resolve => {
        ...
        w.sessionStorage.setItem('psat89_user', 'Test');
...
(async () => {
    ...
    {
        // The case a difference-sniffing heuristic cannot express: an even
        // split. 1:1 with the ratio on must give five and five, not "whatever
        // the queue had".
    ...
    {
        // The mock module is ordered by the SAME function, so the two cannot
        // drift. Whatever MOCK_DOMAIN_MIX says about counts, the module a
        // student sits must still run C&S → I&I → SEC → EoI in blocks.
        w.buildMockExam();
        const m1 = (w.__mockModules() || [])[0] || [];
        const seq = m1.map(q => PEEK.SKILL_DOMAIN[q.skill]);
        const seen = new Set();
        let contiguous = true, prev = null;
        seq.forEach(d => {
            if (d !== prev) { if (seen.has(d)) contiguous = false; seen.add(d); prev = d; }
        });
        ok('the mock module comes out in contiguous domain blocks too',
            m1.length === 27 && contiguous, `${m1.length} questions — ${seq.join(' | ')}`);
        ok('and its blocks are in RW_DOMAIN_ORDER',
            same([...seen], PEEK.RW_DOMAIN_ORDER.filter(d => seen.has(d))),
            [...seen].join(' | '));
    }
